...
use Carbon\Carbon;
use GuzzleHttp\Client; // Pastikan Guzzle diimport
...
class BillingPageKasirController extends Controller
{
    ...
    public function finishBilling(Request $request)
    {
        try {
            ...
            if ($transaction) {
                ...
                if ($transaction->package_name === 'Open Billing') {
                    ...
                    try {
                        ...
                        // Update total revenue di shift terkait
                    ...
                    // Untuk paket biasa, cukup update end_time
                    $transaction->end_time = $endTime;
                    $transaction->save();
                }
            }

            // Update status perangkat menjadi Tersedia dan reset info billing
            ...
            // Panggil FastAPI untuk mematikan perangkat secara otomatis (action off)
            try {
                $client = new Client();
                $response = $client->post('http://127.0.0.1:8001/api/tapo/control', [
                    'json' => [
                ...
                \Log::info('Perintah OFF berhasil dikirim ke FastAPI', [
                    'response' => $response->getBody()->getContents()
                ]);
            } catch (\Exception $e) {
                \Log::error('Gagal mengirim perintah OFF ke FastAPI', ['error' => $e->getMessage()]);
            }
